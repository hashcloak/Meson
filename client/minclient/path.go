...
import (
	...
	kpki "github.com/katzenpost/core/pki"
...
// NewPath creates a new path suitable for use in creating a Sphinx packet with the
...
func selectHops(rng *mRand.Rand, doc *kpki.Document, src, dst *kpki.MixDescriptor, isFromClient, isForward bool) ([]*kpki.MixDescriptor, error) {
	var hops []*kpki.MixDescriptor

	var startLayer, nHops int
	if isForward {
		if dst.Layer != kpki.LayerProvider {
			return nil, fmt.Errorf("path: invalid destination layer: %v", dst.Layer)
		}

		if isFromClient {
			// Client packets must span provider to provider.
			if src.Layer != kpki.LayerProvider {
				return nil, fmt.Errorf("path: invalid source layer: %v", src.Layer)
			}
			nHops = len(doc.Topology) + 2
		} else {
			switch int(src.Layer) {
			case kpki.LayerProvider:
				startLayer = 0
			case len(doc.Topology) - 1:
				return []*kpki.MixDescriptor{dst}, nil
			default:
	...
	hops = make([]*kpki.MixDescriptor, 0, nHops)
